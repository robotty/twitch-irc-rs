...
- Breaking: It was possible to trip up the channel joinery logic of the client pretty good by joining channels with a comma in them. Added a basic validation step to `TwitchIRCClient.login` and `TwitchIRCClient.set_wanted_channels` to ensure channel names are in a sane format. **The methods will now panic if you pass them an invalid channel name.** The underlying used validation function is exported for use as `twitch_irc::validate::validate_login`. Use this function on any user input before passing it on to aforementioned methods.
